import Posts, { find, findOneAndUpdate, findById, aggregate, findOneAndDelete } from "../models/postModel";
import { deleteMany } from "../models/commentModel";
import { find as _find, findOneAndUpdate as _findOneAndUpdate } from "../models/userModel";
...
const postCtrl = {
  ...
  likePost: async (req, res) => {
    try {
      ...
      const like = await findOneAndUpdate(
        { _id: req.params.id },
        {
  ...
  reportPost: async (req, res) => {
    try {
      ...
      const report = await findOneAndUpdate(
        { _id: req.params.id },
        {
  ...
  savePost: async (req, res) => {
    try {
      ...
      const save = await _findOneAndUpdate(
        { _id: req.user._id },
        {
  ...
  getSavePost: async (req, res) => {
    try {
      const features = new APIfeatures(find({_id: {$in: req.user.saved}}), req.query).paginating();
